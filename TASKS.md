...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Original JS/DOM, labels, URI helpers, bounded Function/eval, original UTF-16 regex, for-in, switch and explicit-state expression parsing pass local worker/native/CDP created-form journeys. Default-stack evaluator regressions, all 251 debug tests and 141 selected release tests pass. Real Google still provides no result links; the post-expression checkpoint has allocation-budget errors instead of the prior parser-nesting diagnostic. Next: identify allocation consumption by phase with general diagnostics and independently authored source/AST/runtime fixtures, then optimize genuinely duplicated storage where evidence supports it. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
