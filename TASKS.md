...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Compact AST storage and capacity-aware accounting pass all 368 debug tests, 258 selected release checks and 11 native/11 external CDP local journeys. Statement size is 80 bytes on x86_64, and independent measurements verify real storage charges including holes; the frozen large-function form now fits. Real Google form submission works and search gets past the prior AST rejection, but first reports `Symbol is not defined`; a later script rejects Ast 387,844 after 4,078,595 accepted bytes. No results or destination. Next: design and implement genuine Symbol primitives/property-key behavior against independent language cases, not string shims or success stubs; continue measured cumulative-storage diagnosis separately. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
