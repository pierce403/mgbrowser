...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Original JS/DOM, dynamic compilation, regex, iteration, explicit-state expressions, shared code and prepaid array/argument ownership pass worker/native/CDP created-form journeys. All 312 debug tests and 202 selected release checks pass. The real Google form submits, but search still rejects a 2,575,110-byte AST charge after 1,822,051 accepted bytes: a 202,857-byte gap under the unchanged limit, with no result links. Next: audit narrow paid-value transfers and container-aware AST storage using independent fixtures and measured ownership/layout, preserving real copies and public ingress. Do not simply lower node weights or assume the smaller gap is the last compatibility problem. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
