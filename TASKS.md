...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Original JS/DOM, dynamic compilation, regex, iteration, explicit-state expressions and now shared immutable function code pass worker/native/CDP created-form journeys. All 288 debug tests and 178 selected release tests pass. Fixed-size diagnostics preserve the first rejected allocation; unused native copies and audited prepaid-string recharges are removed with limits unchanged. Real Google homepage no longer exhausts allocation, but search still fails AST admission and has no result links. Next: audit remaining AST representation and runtime ownership/copy costs using independent local fixtures, implementing only demonstrated storage savings. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
