...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Fresh user-function default prototypes now materialize on first value read; real property metadata, unique identity, original constructor, inherited owner and fatal admission remain intact. All 721 debug tests, 623 selected release checks and 17 native/17 external CDP local journeys pass with unchanged limits and Rust-only boundaries. Real Google form submission still works; search first reports a non-callable value, then rejects Source 26,999 after 4,187,629 accepted bytes against the 4,194,304 limit. No results or destination, and no new live journey stage completed. Next: independently authored language/builtin coverage and measured cumulative-storage ownership work before another implementation. The generic diagnostic does not identify the missing callable, changing responses are not a controlled performance comparison, and the last rejected phase need not be the best optimization target. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
