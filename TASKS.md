...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Six existing Error families now have genuine prototypes/instances, ordered string conversion and bounded callback-free diagnostics. All 547 debug tests, 449 selected release checks and 14 native/14 external CDP local journeys pass with unchanged limits and Rust-only boundaries. Real Google form submission still works; the first search error is now unsupported Array.concat instead of the prototype error. Later Ast 387,620 is rejected after 4,132,042 accepted bytes; no results or destination. Next: independently specified bounded Array.concat, with authored semantics/resource and actual-worker baseline cases before implementation, plus separate measured cumulative-storage diagnosis. The changing live response does not prove original arguments or a controlled performance benefit. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
