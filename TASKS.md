...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Typed function/native prototypes, metadata/Symbol inheritance and constructor traversal pass all 492 debug tests, 394 selected release checks and 13 native/13 external CDP local journeys. The frozen prototype-dependent form works with unchanged limits and Rust-only boundaries. Real Google form submission still works, but search retains `TypeError: prototype must be an object or null`; a later script rejects Ast 387,500 after 4,107,727 accepted bytes. No results or destination, and no observed benefit on that leading diagnostic. Next: builtin constructor/prototype coverage, especially the separately reproduced missing Error-family prototypes, and measured cumulative-storage diagnosis. The live message does not establish the supplied prototype or cause. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
