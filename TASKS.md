...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Bounded ES5-shaped Function.bind now preserves receiver/prefix ordering, target call/new/instance delegation and restricted metadata. Independent semantics/resources/private tests and the frozen bound-callback form pass; all 788 debug tests, 690 selected release checks and 18 native/18 external CDP local journeys pass with unchanged limits and Rust-only boundaries. Real Google form submission still works; search first reports property access on null or undefined, then rejects Source 26,794 after 4,191,219 accepted bytes against 4,194,304. No results or destination, and no new live journey stage completed. Next: independently authored language/builtin coverage and measured cumulative-storage ownership work before another implementation. The generic diagnostic does not identify a missing capability, changing responses are not a controlled performance comparison, and the last rejected phase need not be the best optimization target. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
