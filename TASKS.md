...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Original JS/DOM, labels, URI helpers, bounded Function/eval, original UTF-16 regex, for-in and switch now pass local worker/native/CDP created-form journeys. Real Google still provides no result links; the post-iteration checkpoint reaches the parser nesting limit. Next: independently authored nesting cases and stack/resource-safe parser architecture, without blindly raising limits; design parent-brokered external scripts, persistent realms and actual events/timers. Re-run the real journey in bounded checkpoints. Do not port challenge logic, substitute an existing engine or fabricate results.
