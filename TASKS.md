...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Empty-only arguments snapshots now materialize on first read; unused snapshots no longer allocate, while nonempty lists, identity, scope and real admission remain intact. All 659 debug tests, 561 selected release checks and 16 native/16 external CDP local journeys pass with unchanged limits and Rust-only boundaries. Real Google form submission still works; search repeats a Runtime allocation rejection after 4,194,254 accepted bytes, requesting 128 against the 4,194,304 limit. No results or destination, and no new live journey stage completed. Next: independently measured cumulative-storage ownership work using authored baseline cases before implementation. The changing live response is not a controlled performance comparison, and the last rejected phase need not be the best optimization target. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
