...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Genuine Symbol primitives/typed keys, registry/boxing/coercion/reflection and bounded foreign admission pass all 438 debug tests, 340 selected release checks and 12 native/12 external CDP local journeys. The frozen Symbol-dependent form now works; limits and Rust-only boundaries remain intact. Real Google form submission still works, but search first reports `TypeError: prototype must be an object or null`; a later script rejects Ast 387,964 after 4,107,538 accepted bytes. No results or destination. Next: typed function/native prototype identity and traversal, a genuine gap confirmed by independent Object.create/getPrototypeOf/new/instanceof probes; continue measured cumulative-storage diagnosis separately. The live message alone does not establish the supplied prototype or cause. Parent-brokered external scripts, persistent realms and actual events/timers still need designed implementations. Do not blindly raise limits, port challenge logic, substitute an existing engine or fabricate results.
