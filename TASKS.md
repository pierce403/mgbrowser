...
2. **T-007 / F-010, F-008:** Active user goal: open our browser, browse to Google, search, click the first result and attempt the destination. Initial original JS/DOM subset and restricted worker now pass local native/CDP script-created-form journeys. Real Google still provides no result links; the scripted search first reports unsupported labeled-statement syntax. Next: expand general language semantics against independent fixtures, then parent-brokered external scripts, persistent realms and actual events/timers. Re-run the real journey in bounded checkpoints. Do not port challenge logic, substitute an existing engine or fabricate results.
