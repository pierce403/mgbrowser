...
use mg_deps::js::runtime::AllocationReport;
use mg_deps::page_session::{
...
fn child_loop(reader: &mut impl Read, writer: &mut impl Write) -> Result<(), String> {
    ...
    let (mut realm, reply) = mg_deps::js_browser::PageRealm::start(initial);
...
pub fn session_selftest() -> Result<(), String> {
    use mg_deps::page_session::InputKind;
    fn request() -> Request {
...
mod tests {
    use super::*;
    use mg_deps::{
        js::runtime::{AllocationPhase, AllocationTotals, RejectedAllocation},
        page_session::{ArenaSnapshot, ControlAck, ControlEdit, EventOutcome, InputKind},
    };
    use std::io::Cursor;
