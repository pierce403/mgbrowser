...
An original Rust lexer/parser/evaluator and DOM bridge run a limited non-strict, ES5-like subset behind `--enable-scripts`. Scripting is off by default. Linux x86_64 uses a fresh restricted process for each document, with bounded source/DOM/pipes, evaluator budgets, kernel controls and parent deadlines. Inline scripts share a retained realm, can create real controls and change title/content, and have limited startup and later click/submit callbacks. Retained closure state, cancellation and parent-validated handler-updated navigation pass native and independent CDP journeys, alongside readable recovery after a script fuel error. Frames were inspected. This is partial implementation, not language conformance or a general page event loop; see `JAVASCRIPT.md` and `PAGE_SESSIONS.md`.

The real Google goal remains incomplete. Script-disabled browsing loaded and submitted its actual form but reached a JavaScript-required response. With the opt-in interpreter and retained session, the homepage and submitted search again returned HTTP 200; scripts reported unsupported behavior and the page titled “Google Search” had no rendered result items or forms. The journey correctly exited 2. No Google result was fabricated or substituted. External scripts, many language/builtin/DOM behaviors and broader events/timers remain missing; fixing one reported error does not establish compatibility. Full CSS and page-image fetching are also absent.
...
The initial Linux X11/XWayland interface and Linux x86_64 script worker are implemented; additional OS/window/isolation backends require review and must fail closed when unavailable. The TLS/font/image direction is adopted; audit dependency upgrades and any new HTTP/window/raster crates against `DEPENDENCIES.md`. Current project source uses Apache-2.0; preserve third-party licenses and notices when accepting contributions. Preserve the bounded retained-realm ownership contract and design parent-brokered external script fetching before adding it. JavaScript needs continuing language/runtime/conformance gates, not an incidental library swap.
