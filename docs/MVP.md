...
The target is 100% Rust browser code written here, from the ground up. No Chromium, WebKit, Servo embedding, system webview, or existing JavaScript runtime. Proposed boundary: established Rust crates may supply cryptography/TLS, networking utilities, image decoding, font processing, and window/platform interfaces; our HTML parser, DOM, CSS parser/cascade, layout, paint orchestration, navigation, and eventual JS engine remain our own. Native OS/driver interfaces are unavoidable for a desktop application. Audit transitive native implementations before selecting dependencies. A requirement to also replace OS services, TLS, codecs, and every dependency would be a much larger project; this boundary needs confirmation before engine dependencies are adopted.
