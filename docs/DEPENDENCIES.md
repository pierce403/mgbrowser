...
The original interpreter runs eligible inline scripts and startup DOM callbacks in one short-lived realm per document. Browser use requires `--enable-scripts` and a fresh restricted Linux x86_64 worker. The worker has an empty environment, closes inherited descriptors, installs seccomp/resource limits before reading page input, and returns bounded DOM/navigation data to the parent. Its new direct libc dependency supplies Rust declarations for those operating-system calls; no native parser, interpreter, font, image or crypto implementation was added. External script loading and persistent realms are absent. Unsupported isolation fails closed; the renderer and network parent remain outside this boundary.

Requests have a 30-second deadline across HTTP redirects, an 8-second socket/DNS-wait budget per operation, eight HTTP redirects, 64 KiB request/header bounds, and separate 8 MiB encoded/decoded body bounds. The window currently permits two outstanding navigation workers and ignores stale completions. Superseded network operations continue until completion or timeout; cancellation does not interrupt sockets or the platform resolver. A resolver thread can outlive the caller's bounded wait. These transport limits do not constitute network-process isolation or a fully cancellable resource budget. Script workers separately have a two-second wall deadline, one CPU second and 256 MiB address-space cap, plus evaluator/DOM/protocol bounds documented in `JAVASCRIPT.md`.
