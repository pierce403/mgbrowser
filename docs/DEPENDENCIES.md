...
On 2026-09-07, all 15 transport tests passed, including local trusted, unknown-issuer, and hostname-mismatch TLS handshakes, redirect/form behavior, response limits, and cookie scoping. Both sides of the local TLS tests use RustCrypto. The public DER fixtures were generated using OpenSSL's command-line tooling; neither the browser nor its tests call or link OpenSSL. Painter tests cover clipping, coverage blending, and text measurement/painting; the dependency smoke test separately covers PNG round-trip. These component checks do not establish a completed desktop or browser acceptance run.

An ordinary Google homepage GET succeeded with verified TLS. Submitting its actual HTML form, including all successful hidden controls, returned a JavaScript retry document without result links in the initial transport probe. Session-cookie continuity was implemented afterward; the window/document integration and live homepage → search → first-result scenario still require verification. No Google search success is claimed. JavaScript execution, full CSS, and page-image fetching are not implemented; image elements currently have a text/alt placeholder in the window code.

Requests have a 30-second deadline across HTTP redirects, an 8-second socket/DNS-wait budget per operation, eight HTTP redirects, 64 KiB request/header bounds, and separate 8 MiB encoded/decoded body bounds. The window currently permits two outstanding navigation workers and ignores stale completions. Superseded network operations continue until completion or timeout; cancellation does not interrupt sockets or the platform resolver. A resolver thread can outlive the caller's bounded wait. These limits do not constitute process isolation or a fully cancellable resource budget.
