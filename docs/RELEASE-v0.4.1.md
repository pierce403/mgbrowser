# mgbrowser v0.4.1 Experimental Preview

Linux x86_64 / X11 or XWayland, glibc 2.35 or newer. Rust 1.91.1, locked dependencies.

This packaging-only follow-up fixes v0.4.0's download exceeding the existing
8 MiB updater limit in older installed browsers. The release build uses
size-optimized compilation (`opt-level = "s"`) and symbol stripping; a package-size
guard enforces the unchanged download limit. Engine source, dependencies,
resource limits, default LTO/codegen settings and panic behavior are unchanged.
v0.4.0's tag and published assets remain immutable.

Boa 0.22 runs opt-in page JavaScript: inline scripts share modern language state,
Promise checkpoints mutate Mg's real Rust DOM, and retained click/submit handlers
preserve controls and default actions. Butane supplies the execution policy,
Sparkle owns the DOM, and the restricted Linux child contains execution. There
is no original-engine production fallback, JIT, Servo browser embedding or C/C++
JavaScript backend.
...
See [the Boa profile and acceptance boundary](https://github.com/pierce403/mgbrowser/blob/v0.4.1/docs/BOA.md).
Full JSPLAN and formal MVP gates remain open. Project code/artwork use Apache-2.0
with LICENSE/NOTICE. Dependencies retain their licenses: Boa's MIT option and
MPL-2.0 for Stylo-related crates are preserved in THIRD_PARTY_LICENSES.txt with
source archive links. Published v0.1.0/v0.1.1 archives retain their original MIT
licensing.
