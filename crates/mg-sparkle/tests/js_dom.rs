...
fn completed_array_ast_storage_allows_the_frozen_real_form() {
    let html = include_str!("../../../tests/fixtures/script/ast-arrays.html");
    assert!(
...
fn object_create_descriptors_build_the_frozen_real_form() {
    let html = include_str!("../../../tests/fixtures/script/object-create.html");
    assert!(
        document::parse_with_scripting(html, URL, true)
            .forms
            .is_empty()
...
fn static_operator_storage_allows_the_frozen_real_form() {
    let html = include_str!("../../../tests/fixtures/script/static-operators.html");
    assert!(
        mg_sparkle::document::parse_with_scripting(html, URL, true)
            .forms
            .is_empty()
    );
    let reply = execute(Request {
...
fn core_intrinsics_create_the_frozen_real_form() {
    let reply = execute(Request {
        url: URL.into(),
        html: include_str!("../../../tests/fixtures/script/core-intrinsics.html").into(),
    });
    assert!(
        reply.applied && reply.errors.is_empty(),
        "{:?}",
...
fn callback_family_and_borrowed_collections_create_the_frozen_real_form() {
    let reply = execute(Request {
        url: URL.into(),
        html: include_str!("../../../tests/fixtures/script/array-callbacks.html").into(),
    });
    assert!(
        reply.applied && reply.errors.is_empty(),
        "{:?}",
