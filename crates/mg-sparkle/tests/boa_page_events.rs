#![cfg(feature = "modern")]
//! The unchanged retained interaction contract, exercised independently on Boa.
//! Original tests remain intact; only engine-specific counter checks differ.
use mg_sparkle::{
    document::{self, Document},
    js_browser::{Request, boa::BoaPageRealm as PageRealm},
...
fn frozen_fixture_requires_real_later_handlers_and_retains_original_node_ids() {
    let (mut realm, init) = start(include_str!("../../../tests/fixtures/script/events.html"));
    ...
    assert!(submitted.boa.unwrap().opcodes_remaining < init.boa.unwrap().opcodes_remaining);

    let (mut result_realm, init) = start(include_str!(
        "../../../tests/fixtures/script/event-results.html"
    ));
...
#[test]
fn separate_activations_consume_one_cumulative_opcode_budget() {
    let (mut realm, init) = page(
        r#"
...
"#,
    ...
    let mut previous = first.boa.unwrap().opcodes_remaining;
    let mut exhausted = false;
    for _ in 0..7 {
        let reply = click(&mut realm, target);
        let accepted = reply.boa.unwrap().opcodes_remaining;
        assert!(
            accepted <= previous,
            "input must not renew opcode accounting"
        );
        previous = accepted;
        if reply.state == RealmState::Fatal {
            assert!(reply.errors.iter().any(|error| error.contains("budget")));
            assert_eq!(reply.default_action, DefaultAction::None);
            exhausted = true;
            break;
        }
