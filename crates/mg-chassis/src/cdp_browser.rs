...
impl App {
    ...
    pub(crate) fn viewport_height(&self) -> u32 {
        self.height
            .saturating_sub(self.page_top() as u32 + self.page_footer())
    }
            ...
            for _ in 0..=256 {
                ...
                if source == node {
                    let (x, y, right, bottom) = (
                        rect.x,
                        rect.y - self.page_top(),
                        rect.x + rect.width as i32,
                        rect.y - self.page_top() + rect.height as i32,
                    );
...
mod tests {
    ...
    fn toolbar_free_cdp_uses_the_full_page_surface_and_coordinates() {
        ...
        let rect = app.boxes.iter().find(|rect| rect.node == node).unwrap();
        let model = app.box_model(node).unwrap();
        assert_eq!(model["model"]["content"][1], rect.y);
        assert_eq!(app.metrics()["cssLayoutViewport"]["clientHeight"], 480);
    ...
    fn immediate_activation_rejection_is_a_protocol_error_not_dropped_success() {
        use mg_sparkle::page_session::{InputKind, SessionInput};
    ...
    fn metadata_uses_committed_page_and_every_advertised_method_is_recognized() {
        ...
        let version = cdp
            ...
            .unwrap();
        assert_eq!(version["userAgent"], crate::net::USER_AGENT);
        let schema: Value =
